...
def main(im_path:str,FRAP_frame:str,size_of_bbox_zoom:int,
    ...
    c = [int(x) for x in list(coords[0])]
    center = c 
    size = size_of_bbox_zoom 
    ...
    logger.info(f"The shape of the image analyzed centered at {coords} is {im_r.shape}\n")
    ...
    for i in range(0,im_r.shape[0]):
        ...
        try:
            ...
            df.to_csv(save_path.split('/')[-1]+f'_raw_v2_stopped_at_{i}.csv')
    ...
    logger.info(f'The coordinates of the ROI for cell {index} are {coords_ROI}\n')
    ...
    df.rename(columns={'Unnamed: 0':'time'},inplace=True)
    df.to_csv(save_path.split('/')[-1]+'_raw_v2.csv')
